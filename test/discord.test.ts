...
import { Spring } from '../src/domain/season.js'
...
describe('integration', { skip: !token, timeout: timeout }, () => {
    ...
    it('should set presence', async () => {
        const season = { season: Spring, currentDay: 5, totalDays: 11 }
        const players: Player[] = [{ username: 'player_name', character: 'Wilson' }]
        const presence = new Presence(season, players, Date.now(), Date.now() + updateFrequency)

        const clientPresence = discord.setPresence(presence)

        assert.equal(clientPresence.status, 'online')
        assert.equal(clientPresence.activities[0].state, '🌸Spring day 5/11 🧍1')
    })
})
